...
<%
	...
	if(userID == null){
		...
		String result = dao.Certification(name,hp,userID);
		
		if(result.equals("")) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('이름과 전화번호와 이메일이 일치하지 않습니다');");
		...
		String from = "ssangyongsubwayproject@gmail.com";
		String to = result;
		String subject = "Subway Password인증을 위한 이메일 입니다.";
		String content = "다음 링크에 접속하여 이메일 확인을 진행하세요" + 
			"<a href='" + host + "emailCheckAction.jsp?id=" + userID + "&code=" + new SHA256().getSHA256(to) + "'>이메일 인증하기</a>";
		Properties p = new Properties();
		p.put("mail.smtp.user", from);
		p.put("mail.smtp.host", "smtp.googlemail.com");
		p.put("mail.smtp.port", "465");
